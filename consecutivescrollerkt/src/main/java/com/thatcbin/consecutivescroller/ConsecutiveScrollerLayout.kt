...
import com.thatcbin.consecutivescroller.LayoutParamsUtils.invalidTopAndBottomMargin
import com.thatcbin.consecutivescroller.ScrollUtils.canScrollVertically
import com.thatcbin.consecutivescroller.ScrollUtils.computeVerticalScrollOffset
import com.thatcbin.consecutivescroller.ScrollUtils.computeVerticalScrollRange
import com.thatcbin.consecutivescroller.ScrollUtils.getRawX
import com.thatcbin.consecutivescroller.ScrollUtils.getRawY
import com.thatcbin.consecutivescroller.ScrollUtils.getScrollBottomOffset
import com.thatcbin.consecutivescroller.ScrollUtils.getScrollTopOffset
import com.thatcbin.consecutivescroller.ScrollUtils.getScrolledView
import com.thatcbin.consecutivescroller.ScrollUtils.isConsecutiveScrollParent
import com.thatcbin.consecutivescroller.ScrollUtils.isConsecutiveScrollerChild
import com.thatcbin.consecutivescroller.ScrollUtils.isHorizontalScroll
import com.thatcbin.consecutivescroller.ScrollUtils.isTouchNotTriggerScrollStick
import com.thatcbin.consecutivescroller.ScrollUtils.isTouchPointInView
import com.thatcbin.consecutivescroller.ScrollUtils.startInterceptRequestLayout
import com.thatcbin.consecutivescroller.ScrollUtils.stopInterceptRequestLayout
import java.util.*

class ConsecutiveScrollerLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ViewGroup(context, attrs, defStyleAttr), ScrollingView, NestedScrollingParent2, NestedScrollingChild2 {
    /**
    ...
    override fun addView(child: View, index: Int, params: ViewGroup.LayoutParams) {
        ...
        if (isConsecutiveScrollerChild(child)) {
            ...
            if (child is IConsecutiveScroller) {
                val views = (child as IConsecutiveScroller).scrolledViews
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(measureSize(widthMeasureSpec, contentWidth + paddingLeft + paddingRight),
     ...
     * 获取子view的left
     */
    private fun getChildLeft(child: View, parentWidth: Int, paddingLeft: Int, paddingRight: Int): Int {
    ...
    override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                isTouchNotTriggerScrollStick = isTouchNotTriggerScrollStick(this, mDownLocation[0], mDownLocation[1])
            }
            ...
            MotionEvent.ACTION_CANCEL, MotionEvent.ACTION_UP -> {
                if (mAdjustVelocityTracker != null) {
                    mAdjustVelocityTracker!!.addMovement(vtev)
                    mAdjustVelocityTracker!!.computeCurrentVelocity(1000, mMaximumVelocity.toFloat())
                    ...
                    val canScrollVerticallyChild = canScrollVertically(getTouchTarget(touchX, touchY)!!)
                    if (SCROLL_ORIENTATION != SCROLL_VERTICAL && canScrollVerticallyChild
                            && Math.abs(yVelocity) >= mMinimumVelocity && !isHorizontalScroll(this, touchX, touchY)) {
                        //如果当前是横向滑动，但是触摸的控件可以垂直滑动，并且产生垂直滑动的fling事件，
                        // 为了不让这个控件垂直fling，把事件设置为MotionEvent.ACTION_CANCEL。
                        ev.action = MotionEvent.ACTION_CANCEL
                    }
                    if (SCROLL_ORIENTATION == SCROLL_NONE && !isConsecutiveScrollParent(this)
    ...
    override fun onTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.actionMasked) {
            ...
            MotionEvent.ACTION_POINTER_DOWN, MotionEvent.ACTION_POINTER_UP -> mTouchY = ev.getY(pointerIndex).toInt()
            MotionEvent.ACTION_MOVE -> {
                if (mTouchY == 0) {
                    mTouchY = ev.getY(pointerIndex).toInt()
                    return true
        ...
        do {
            ...
            if (!isScrollBottom) {
                ...
                if (firstVisibleView != null) {
                    awakenScrollBars()
                    val bottomOffset = getScrollBottomOffset(firstVisibleView)
                    if (bottomOffset > 0) {
                        scrollOffset = Math.min(remainder, bottomOffset)
                        if (mScrollToIndex != -1) {
                            scrollOffset = Math.min(scrollOffset, scrollAnchor - (scrollY + paddingTop + viewScrollOffset))
                        }
        ...
        do {
            ...
            if (!isScrollTop) {
                ...
                if (lastVisibleView != null) {
                    awakenScrollBars()
                    val childScrollOffset = getScrollTopOffset(lastVisibleView)
                    if (childScrollOffset < 0) {
                        scrollOffset = Math.max(remainder, childScrollOffset)
                        if (mScrollToIndex != -1) {
                            scrollOffset = Math.max(scrollOffset, scrollAnchor - (scrollY + paddingTop + viewScrollOffset))
                        }
                        ...
                        if (mScrollToIndex != -1) {
                            scrollOffset = Math.max(scrollOffset, scrollAnchor - (getScrollY() + paddingTop + viewScrollOffset))
                        }
                        scrollSelf(scrollY + scrollOffset)
                    }
                    mSecondScrollY += scrollOffset
    ...
    private fun scrollChild(child: View?, y: Int) {
        ...
        if (scrolledView is AbsListView) {
            ...
            if (scrolledView is RecyclerView) {
                isInterceptRequestLayout = startInterceptRequestLayout(scrolledView)
            }
     ...
     * @param target
     */
    private fun scrollChildContentToBottom(target: View?) {
        var scrollY = 0
        do {
            scrollY = 0
...
//        // 导致ConsecutiveScrollerLayout追踪布局的滑动出现偏差
    ...
    val isScrollTop: Boolean
        get() {
            ...
            if (size > 0) {
                ...
                val isScrollTop = scrollY <= 0 && !canScrollVertically(child, -1)
                if (isScrollTop) {
                    for (i in 0 until size) {
                        val view = children[i]
                        if (view is ConsecutiveViewPager) {
                            val viewPager = view
                            if (viewPager.adjustHeight > 0 && isConsecutiveScrollerChild(viewPager)
                                    && canScrollVertically(viewPager, -1)) {
                                return false
                            }
                        }
                    }
    ...
    private fun isIntercept(ev: MotionEvent): Boolean {
        ...
        return if (pointerIndex < 0 || pointerIndex >= ev.pointerCount) {
            // 无效的触摸，不要往下传递
            true
        } else isIntercept(getRawX(this, ev, pointerIndex),
    ...
    private fun isIntercept(touchX: Int, touchY: Int): Boolean {
        ...
        return if (target != null) {
            isConsecutiveScrollerChild(target)
        } else false
    }
    ...
    private fun getViewsScrollOffset(index: Int): Int {
        ...
        for (i in index until count) {
            val child = children[i]
            if (child.visibility != GONE && isConsecutiveScrollerChild(child)) {
