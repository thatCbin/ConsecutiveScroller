...
object ScrollUtils {
    ...
    fun getScrollBottomOffset(view: View): Int {
        return if (isConsecutiveScrollerChild(view) && canScrollVertically(view, 1)) {
            Math.max(computeVerticalScrollRange(view) - computeVerticalScrollOffset(view)
        ...
        return if (scrolledView is AbsListView) {
            ...
            if (scrolledView is RecyclerView) {
                ...
                if (layoutManager != null && adapter != null && adapter.itemCount > 0) {
                    val itemView = layoutManager.findViewByPosition(if (direction > 0) adapter.itemCount - 1 else 0)
                            ?: return true
                } else {
                    return false
                }
    ...
    fun getTouchViews(rootView: View, touchX: Int, touchY: Int): List<View> {
        val views: MutableList<*> = ArrayList<Any?>()
        addTouchViews(views as MutableList<View>, rootView, touchX, touchY)
        return views
    }
    ...
    fun stopInterceptRequestLayout(view: RecyclerView) {
        if ("InterceptRequestLayout" == view.tag) {
            try {
                val method = RecyclerView::class.java.getDeclaredMethod("stopInterceptRequestLayout", Boolean::class.javaPrimitiveType)
                method.isAccessible = true
     ...
     * @return
     */
    fun getInTouchCSLayout(rootView: View, touchX: Int, touchY: Int): List<ConsecutiveScrollerLayout> {
    ...
    fun getTopViewInTouch(csl: ConsecutiveScrollerLayout, touchX: Int, touchY: Int): View? {
